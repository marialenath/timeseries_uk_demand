...
nn_x =[  'weekday', 'time','month', 'temp_lhr_min', 
                         'temp_lhr_prev_h', 'school_hol', 'tempk_heathrow',
                         'tempk_coleshill', 'rad_jm2_heathrow', 'rad_jm2_devon_max3',
                         'load_prev_h48', 'tempk_bramham', 'rad_jm2_lhr_max3', 'tempk_devon', 
                         'rad_jm2_coleshill', 'tempk_bishopton', 'rad_jm2_devon',
                         'rad_jm2_bramham', 'wind_mps_bramham', 'wind_mps_bishopton']


features_to_be_scaled = [ 'weekday', 'time','month', 'temp_lhr_min', 
                         'temp_lhr_prev_h', 'tempk_heathrow',
                         'tempk_coleshill', 'rad_jm2_heathrow', 'rad_jm2_devon_max3',
                         'load_prev_h48', 'tempk_bramham', 'rad_jm2_lhr_max3', 'tempk_devon', 
                         'rad_jm2_coleshill', 'tempk_bishopton', 'rad_jm2_devon',
                         'rad_jm2_bramham', 'wind_mps_bramham', 'wind_mps_bishopton']
...
timesteps=48
model = Sequential()
model.add(LSTM(new_nntrainx.shape[2], activation='relu', input_shape=(timesteps,new_nntrainx.shape[2])))
model.add(RepeatVector(timesteps))
model.add(LSTM(new_nntrainx.shape[2], activation='relu', return_sequences=True))
#model.add(TimeDistributed(Dense(20, activation='relu'))) 
model.add(TimeDistributed(Dense(1)))
model.compile(optimizer='adam', loss='mse')

model.fit(new_nntrainx, new_nntrainy, epochs=200, validation_split=0.2, verbose=1, batch_size=32)
res = model.predict(new_nntestx, verbose=0)
model.summary()

test.loc[:, 'pca_lstm_ae'] = np.exp(yscaler.inverse_transform(res.reshape(res.shape[0]*res.shape[1], 1)))
stats = get_stats(test, model_name = 'pca_lstm_ae')
stats.to_csv('pca_lstm_ae_stats.csv', index=False)
test[['dt', 'time', 'load_act', 'pca_lstm_ae']].to_csv('pca_lstm_ae.csv', index=False)

# save model
filename = 'pca_lstm_ae.sav'
joblib.dump(model, filename)


# load the model from disk
loaded_model = joblib.load(filename)
res = loaded_model.predict(new_nntestx, verbose=0)
test.loc[:, 'pca_lstm_ae_loaded'] = np.exp(yscaler.inverse_transform(res.reshape(res.shape[0]*res.shape[1], 1)))

from keras.utils.vis_utils import plot_model
plot_model(model, to_file='pca_lstm_ae.png', show_shapes=True, show_layer_names=True)